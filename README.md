...
Web 导出没有系统字体回退，因此项目默认 UI 字体为 `assets/fonts/NotoSansSC-Game.ttf`（Noto Sans SC 子集：中英 UI 文案 + GB2312 一级汉字，覆盖常见角色名）。若新增大量中文文案后出现缺字，用同目录 `regen_subset.py` 从完整 OTF 重新生成子集。
...
Web 包约二十多 MB（压缩后），手机首次打开可能要几分钟（下载 + WASM 编译），进度条下会提示当前阶段；建议用 Wi‑Fi，电脑浏览器通常更快。手机 / 触屏会显示虚拟摇杆；电脑用键盘或手柄。

详见 [2dog Web 文档](https://2dog.dev/web.html)。

## 核心玩法

- **创建角色**：输入角色名并选择英雄（战士 / 法师 / 猎人），再选择关卡与难度进入；首次选定「起始英雄」并解锁，其余英雄用元货币解锁  
- **局外升级**：用元货币提升各英雄局外等级；每级提高局内初始攻击（约 +10%/级）；达到 Lv3 / Lv6 解锁主动技能（Q / E，带冷却）  
- **难度**：普通 / 困难 / 噩梦 —— 刷怪更快、怪物生命与攻击更高，结算货币也更多  
- **操作**：电脑 WASD / 方向键移动，空格或 Shift 冲刺，Q / E 释放已解锁技能；手柄左摇杆 / 十字键移动，A 或 RB 冲刺，X / Y 技能；手机 / 触屏显示虚拟摇杆、冲刺与技能键；武器与宠物自动攻击  
...
已落地：局外角色升级、技能解锁（Q/E）、普通/困难/噩梦难度。

## 已知限制（原型）

- 广告解锁槽位仅为本地占位，无真实广告  
- 无完整音效与关卡编辑器内容  
- 存档仅本地 `user://save.cfg`（起始英雄、解锁、元货币、局外等级、难度）  
